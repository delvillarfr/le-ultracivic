...
class Settings(BaseSettings):
    ...
    thirdweb_secret_key: str = Field(
        ..., description="Thirdweb secret key for treasury operations"
    )
    treasury_wallet_address: str = Field(
        ..., description="Ultra Civic treasury wallet address"
    )
    pr_token_contract_address: str = Field(
        ..., description="$PR token contract address on Sepolia"
    )
    ...
    alert_email: str = Field(
        default="paco@ultracivic.com", description="Email for alerts"
    )
    ...
    rate_limit_requests: int = Field(
        default=10, description="Rate limit requests per minute per IP"
    )

    # Transaction Monitoring
    tx_timeout_minutes: int = Field(
        default=5, description="Transaction timeout in minutes"
    )
    ...
    allowance_price_usd: float = Field(
        default=24.0, description="Price per allowance in USD"
    )
    ...
    cors_allow_all_origins: bool = Field(
        default=False, description="Allow CORS from any origin"
    )
...
def validate_environment():
    ...
    try:
        ...
        if settings.debug and not settings.database_url.startswith(
            "postgresql+asyncpg://localhost"
        ):
            logging.warning(
                "Debug mode enabled with non-local database. This may expose sensitive data."
            )
